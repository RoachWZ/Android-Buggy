...
public class BLEActivity extends AppCompatActivity {
    ...
    private static final String TAG = HandGestureActivity.class.getSimpleName();
    /**
     * 请求打开蓝牙
     */
    ...
    private void connectDevice(BleDevice bleDevice) {
        ...
        //跳转页面
        Intent intent = new Intent(this, HandGestureActivity.class);
            ...
            public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
                switch (newState) {
                    case BluetoothProfile.STATE_CONNECTED://连接成功
                        isConnected = true;
                        Log.d(TAG, device.getAddress()+"连接成功");
                        runOnUiThread(() -> {
                            layConnectingLoading.setVisibility(View.GONE);
                            showMsg(device.getAddress()+"连接成功");
                            //放入缓存
